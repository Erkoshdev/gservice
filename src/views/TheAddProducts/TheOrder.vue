    ...
    <div class="internal-wrapper" v-if="!result.title">
      ...
      <div class="redact">
        ...
        <div class="redact-container redact-form">
          ...
          <app-input :label="$t('what_should_be_done')" type="text" v-model="title"></app-input>
          <app-input :label="$t('address')" type="text" v-model="address"></app-input>
          <app-input :label="$t('price')" type="number" v-model="price"></app-input>
          <app-select :label="$t('order_type')" :options="typeCodes" label-name="title" v-model="type_code" :reduce="item => item.type"></app-select>
          <app-datepicker type-date-picker="datetime" :default-data="defaultDate" :placeholder="$t('choose_time')" :clearable="true" format="YYYY-MM-DD HH:MM" v-model="start_datetime" />
        </div>
        <div class="redact-container redact-upload" v-if="id">
          <b class="redact-container-title">{{ $t('order_photo') }}</b>
        ...
        <div class="redact-container redact-upload">
          <b class="redact-container-title">{{ $t('upload_photo') }}</b>
          <div class="redact-upload-list">
...
export default {
  ...
  data() {
    return {
      ...
      typeCodes: [
        {
          type: 'by_hour',
          title: this.$t('by_the_hour'),
        },
        {
          type: 'for_longtime',
          title: this.$t('long_term'),
        },
      ],
    }
  },
  ...
  methods: {
    ...
    goPage() {
      this.$router.push({ name: 'TheFilterOrder' })
    },
    onFileChange(e) {
      e.preventDefault();
    ...
    fetchData() {
      if (this.id) {
        ...
        API.product.getOrder({ params }).then((res) => {
          this.region_id = res.order.region.id
          this.address = res.order.address
          this.title = res.order.title
          this.price = parseInt(res.order.price)
          this.type_code = res.order.type.code
          this.images = res.order.images
          this.defaultDate = this.$moment(res.order.start_datetime).format("YYYY-MM-DD HH:MM")
          this.start_datetime = this.$moment(res.order.start_datetime).format("YYYY-MM-DD HH:MM")
        });
      }
    },
    handleSave() {
      ...
      formData.append("status_code", this.status_code);
      for (var i = 0; i < this.uploadFiles.length; i++) {
        let file = this.uploadFiles[i];
        formData.append('images[' + i + ']', file);
      }
      if (!this.id) {
        API.productAdd.order(formData).then((res) => {
          this.$store.commit('setSpinnerLoader', false);
          if (res.success) {
            this.result.title = res.order.title;
            this.result.text = res.order.address;
            this.result.images = res.order.images;
          }
          else {
        ...
        API.productAdd.orderUpdate(formData).then((res) => {
          if (res.success) {
            this.result.title = res.order.title;
            this.result.text = res.order.address;
            this.result.images = res.order.images;
