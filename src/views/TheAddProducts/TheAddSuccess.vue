<template>
  <div class="page-wrapper add-advert-page">
    <div class="internal-wrapper">
      <div></div>
      <div class="redact redact-success">
        <div class="redact-title">
          <router-link class="redact-back-link" to="/">
        ...
        <div class="redact-container">
          <div class="success-container">
            ...
            <p> {{ $t('successfully_created') }}</p>
            <b> {{ $t('congratulations') }}</b>
          </div>
          <div class="redact-ready-card">
            <div class="ready-card-img" v-if="item?.images">
              <img :src="item?.images[0].url" alt="">
            </div>
            <div class="ready-card-caption">
              <b>{{item.title}}</b>
              <p>{{item.text}} </p>
            </div>
          </div>
        </div>
        <button class="text-link-blue" @click="  reload" >+ {{ $t('add_more') }}</button>
        <app-button class="redact-btn" type="btn-warning" size="btn-md" @action="$emit('goPage')" > {{ $t('go') }} </app-button>
      </div>
      <div></div>
    </div>
  </div>
