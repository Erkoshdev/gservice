<template>
    <div class="page-wrapper add-advert-page">
      <div class="internal-wrapper" v-if="!result.title">
        ...
        <div class="redact">
          ...
          <div class="redact-container redact-form">
            ...
            <app-select :label="$t('Fuel_type')" :options="fules" label-name="title" v-model="fuel_type_id" :reduce="item => item.id"></app-select>
            <app-select :label="$t('Condition')" :options="vehicleStatus" label-name="title" v-model="vehicle_status_id" :reduce="item => item.id"></app-select>
            <app-select :label="$t('availability')" :options="sctockList" label-name="title" v-model="in_stock" :reduce="item => item.id"></app-select>
            <app-input :label="$t('price')" type="number" v-model="price"></app-input>
            <app-datepicker :default-data="defaultDate" type-date-picker="year" :placeholder="$t('Select_year')" :clearable="true" format="YYYY" v-model="year" />
          </div>
          ...
          <div class="redact-container redact-text">
            <b class="redact-container-title">{{ $t('advert_desc') }}</b>
            <textarea :placeholder="$t('write_desc')" rows="6" v-model="description"></textarea>
          </div>
          <label class="checkbox-container" v-if="!id">
            <i18n path="agree_with_the_placement_rules">
              <a place="link" href="https://docs.google.com/document/d/1EAdkeFX_gHRr9AnEKfGLVeEYGfdY-Yeg/edit" target="_blank" style="color: #2787F5">{{ $t('with_advert_rules') }}</a>
            </i18n>
            <input  type="checkbox" value="1" v-model="agreed" v-if="!this.id">
      ...
      <div></div>
      <the-add-success v-if="result.title" @goPage="goPage" :item="result" />
  </div>
</template>
  ...
  data() {
    return {
      ...
      sctockList: [
        {
          id: 1,
          title: this.$t('To_order'),
        },
        {
          id: 2,
          title: this.$t('In_stock'),
        },
  ...
  methods: {
    ...
    fetchData() {
      if (this.id) {
        ...
        API.product.getVehicle({ params }).then((res) => {
          this.region_id = res.sale_vehicle.region.id;
          this.vehicle_type_id = res.sale_vehicle.type.id
          this.mark_id = res.sale_vehicle.mark.id
          this.model = res.sale_vehicle.model
          this.fuel_type_id = res.sale_vehicle.fuel.id
          this.in_stock = res.sale_vehicle.in_stock + 1;
          this.vehicle_status_id = res.sale_vehicle.status.id;
          this.price = res.sale_vehicle.price;
          this.defaultDate = res.sale_vehicle.year.toString();
          this.year = res.sale_vehicle.year;
          this.images = res.sale_vehicle.images;
          this.description = res.sale_vehicle.description;
        });
      }
    },
    goPage() {
      this.$router.push({ name: 'TheFilterVehicle' })
    },
    handleSave() {
      if (this.agreed) {
        this.agreed = 1
      ...
      formData.append("vehicle_status_id", this.vehicle_status_id);
      formData.append("mark_id", this.mark_id);
      formData.append("model", this.model);
      formData.append("fuel_type_id", this.fuel_type_id);
      formData.append("in_stock", this.in_stock - 1);
      ...
      if(this.agreed){
        ...
        if (!this.id) {
          API.productAdd.vehicleSale(formData).then((res) => {
            if (res.success) {
              this.result.title = res.sale_vehicles.type.title;
              this.result.text = res.sale_vehicles.mark.id;
              this.result.images = res.sale_vehicles.images;
            }
            else {
          ...
          formData.append("sale_vehicle_id", this.id);

          API.productAdd.vehicleUpdate(formData).then((res) => {
