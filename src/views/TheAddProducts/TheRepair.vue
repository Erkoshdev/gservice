<template>
      ...
      <div class="internal-wrapper"  v-if="!result.title">
        ...
        <div class="redact">
          ...
          <div class="redact-container redact-form">
            ...
            <app-select :label="$t('Profession')" :options="professions" label-name="title" v-model="profession_ids" :reduce="item => item.id"></app-select>
            <app-input :label="$t('Work_experience')" type="number" v-model="experience"></app-input>
          </div>
          <div class="redact-container redact-text">
            <b class="redact-container-title">{{ $t('advert_desc') }}</b>
            <textarea :placeholder="$t('write_desc')" rows="6" v-model="about_me"></textarea>
          </div>
  ...
  export default {
    ...
    methods: {
      goPage(){
        this.$router.push({name: 'TheFilterRepair'})
      },
      fetchData(){
        if(this.id){
          const params = {
          ...
          API.product.getRepair({params}).then((res) => {
            this.region_id = res.repairmen.region.id
            this.profession_ids = res.repairmen.profession[0].id
            this.experience = res.repairmen.experience
            this.about_me = res.repairmen.about_me
          });
        }
      },
      handleSave(){
        ...
        formData.append("profession_ids", this.profession_ids);
        formData.append("experience", this.experience);
        formData.append("about_me", this.about_me);
        API.productAdd.repair(formData).then((res) => {
          this.$store.commit('setSpinnerLoader', false);
            if(res.success){
              this.result.title = res.repairmen.region.title;
              this.result.text = res.repairmen.experience;
            }
            else{
