    ...
    <div class="internal-wrapper" v-if="!result.title">
      ...
      <div class="redact">
        ...
        <div class="redact-container redact-form">
          <app-input :label="$t('Product_Name')" type="text" v-model="name"></app-input>
          <app-input :label="$t('part_number_order')" type="text" v-model="part_number"></app-input>
          <app-select :label="$t('Category')" :options="categories" label-name="title" v-model="category_id"
            :reduce="item => item.id"></app-select>
          <app-select :label="$t('production')" :options="countries" label-name="title" v-model="country_id"
            :reduce="item => item.id"></app-select>
          <app-select :label="$t('Goods_for_transport')" :options="marks" label-name="title" v-model="mark_id"
            :reduce="item => item.id"></app-select>
          <app-input :label="$t('price')" type="text" v-model="price"></app-input>
          <app-select :label="$t('availability')" :options="sctockList" label-name="title" v-model="in_stock"
            :reduce="item => item.id"></app-select>
        </div>
        <div class="redact-container redact-upload" v-if="id">
          <b class="redact-container-title">{{ $t('shop_item_photo') }}</b>
        ...
        <div class="redact-container redact-upload">
          <b class="redact-container-title">{{ $t('upload_shop_item_photo') }}</b>
          <div class="redact-upload-list">
  ...
  methods: {
    ...
    fetchData() {
      if (this.id) {
        ...
        API.product.getShop({ params }).then((res) => {
          this.category_id = res.item.category.id
          this.country_id = res.item.country.id
          this.mark_id = res.item.mark.id
          this.name = res.item.name
          this.part_number = res.item.part_number
          this.price = res.item.price

          if (res.item.in_stock === 'На заказ') {
            this.in_stock = 1;
          } else {
            this.in_stock = 2;
          }
          this.images = res.item.images;
    ...
    handleSave() {
      ...
      if (!this.id) {
        API.productAdd.shopItem(formData).then((res) => {
          this.$store.commit("setSpinnerLoader", false);
          if (res.success) {
            this.result.title = res.item.name;
            this.result.text = res.item.mark.title;
            this.result.images = res.item.images;
          } else {
        ...
        API.productAdd.shopItemUpdate(formData).then((res) => {
           this.$store.commit("setSpinnerLoader", false);
          if (res.success) {
            this.result.title = res.item.name;
            this.result.text = res.item.mark.title;
