        ...
        <div class="product-container">
          <div class="product-container-top">
            ...
            <div class="product-favourite" v-if="$store.getters.loggedIn && !my">
              <input type="checkbox" @change="handleLike(shop)" :checked="shop.is_favourite">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div class="product-container-content">
            <div class="sale-container">
              <div class="product-gallery">
                ...
                >
                  <div class="gallery-item" v-for="(img, imgIdx) in shop.images" :key="imgIdx">
                    <img :src="img.url" alt="">
              ...
              <div class="sale-name">{{ shop.name }}</div>
              <div class="sale-price" v-if="shop.price">{{ shop.price }}тг</div>
              <div class="sale-detail-list">
                <div class="sale-detail-item">
                  <div class="sale-detail-name">{{ $t('city') }}</div>
                  <div class="sale-detail-value">{{ shop.shop.city }}</div>
                </div>
                <div class="sale-detail-item">
                  <div class="sale-detail-name">{{ $t('For_transport') }}</div>
                  <div class="sale-detail-value">{{ shop.mark.title }}</div>
                </div>
                <div class="sale-detail-item">
                  <div class="sale-detail-name">{{ $t('production') }}</div>
                  <div class="sale-detail-value">{{ shop.country.title }}</div>
                </div>
                <div class="sale-detail-item">
                  <div class="sale-detail-name">{{ $t('Category') }}</div>
                  <div class="sale-detail-value">{{ shop.category.title }}</div>
                </div>
              </div>
              <div class="sale-info">
          ...
          <div class="product-tools-body">
            <div class="owner-badge"  v-if="!my" >
              ...
              <div class="owner-badge-caption">
                <p>{{ shop.shop.name }}</p>
                <span>{{ shop.created_at | moment("DD.MM.YYYY HH:mm") }}</span>
              </div>
            </div>
            <app-button class="show-phone" type="btn-success"  v-if="!my"  size="btn-lg" @action="handleShowNumber">
              <a :href="`tel:${shop.shop.phone}`" v-if="isNumber">{{ shop.shop.phone }}</a>
              <template v-else>{{ $t('show_number') }}</template>
            </app-button>
            <router-link class="btn-plain order-control-btn" :to="{name: 'TheShopItemEdit', params: {id: shop.id}}"   v-if="my">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <button class="btn-plain order-control-btn" v-if="my"   @click="remove(shop.id)" >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default {
  ...
  methods: {
    remove(id){
      API.productAdd.removeShopItem({ id: id}).then((res) => {
        if(res.success) {
          this.$router.push('/');
