    ...
    <div class="product-wrapper">
          ...
          <div class="product-container-content">
            <div class="sale-container">
              <div class="product-gallery">
                ...
                >
                  <div class="gallery-item" v-for="(img, imgIdx) in vehicle.images" :key="imgIdx">
                    <img :src="img.url" alt="">
              ...
              <div class="sale-name">{{ vehicle.mark.title }} {{ vehicle.model }}</div>
              <div class="sale-price">{{ vehicle.price }}тг</div>
              <div class="sale-detail-list">
                <div class="sale-detail-item">
                  <div class="sale-detail-name">{{ $t('model') }}</div>
                  <div class="sale-detail-value">{{ vehicle.model }}</div>
                </div>
                <div class="sale-detail-item">
                  <div class="sale-detail-name">{{ $t('city') }}</div>
                  <div class="sale-detail-value">{{ vehicle.region.title }}</div>
                </div>
                <div class="sale-detail-item">
                  <div class="sale-detail-name">{{ $t('year') }}</div>
                  <div class="sale-detail-value">{{ vehicle.year }}</div>
                </div>
                <div class="sale-detail-item">
                  <div class="sale-detail-name">{{ $t('Fuel_type') }}</div>
                  <div class="sale-detail-value">{{ vehicle.fuel.title }}</div>
                </div>
                <div class="sale-detail-item">
                  <div class="sale-detail-name">{{ $t('Condition') }}</div>
                  <div class="sale-detail-value">{{ vehicle.status.title }}</div>
                </div>
              </div>
              <div class="sale-info">
                <b>{{ $t('Seller_comment') }}</b>
      ...
      <div class="product-sidebar">
        <div class="product-tools">
          <div class="product-tools-header">
            <p>{{ $t('owner') }}</p>
          </div>
          <div class="product-tools-body">
            ...
            <app-button class="show-phone" type="btn-success" size="btn-lg" @action="handleShowNumber"  v-if="!my">
              <a :href="`tel:${vehicle.user.phone}`" v-if="isNumber">{{ vehicle.user.phone }}</a>
              <template v-else>{{ $t('show_number') }}</template>
            </app-button>
            <router-link class="btn-plain order-control-btn" :to="{name: 'TheVehicleEdit', params: {id: vehicle.id}}"   v-if="my">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <button class="btn-plain order-control-btn" v-if="my"   @click="remove(vehicle.id)" >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default {
  ...
  methods: {
    remove(id){
      API.productAdd.removeVehicle({ id: id}).then((res) => {
        if(res.success) {
          this.$router.push('/');
    ...
    fetchData() {
      ...
      API.product.getVehicle({params})
          .then((res) => {
            this.vehicle = res.sale_vehicle;
          })
          .catch((error) => {
            console.log(error);
          })
