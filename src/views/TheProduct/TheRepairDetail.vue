        ...
        <div class="product-container">
          <div class="product-container-top">
            ...
            <div class="product-favourite" v-if="$store.getters.loggedIn && !my">
              <input type="checkbox" @change="handleLike(repair)" :checked="repair.is_favourite">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div class="product-container-content">
            <div class="sale-container">
              <div class="sale-price">{{ name(repair.profession) }}</div>
              <div class="sale-detail-list">
                <div class="sale-detail-item">
                  <div class="sale-detail-name">{{ $t('city') }}</div>
                  <div class="sale-detail-value">{{ repair.region.title }}</div>
                </div>
                <div class="sale-detail-item">
                  <div class="sale-detail-name">{{ $t('experience_single') }}</div>
                  <div class="sale-detail-value">{{ repair.experience }}
                    {{$i18n.locale === 'ru' ? 'лет' : $i18n.locale === 'en' ? 'year' : 'жыл'}}
                  </div>
                </div>
              </div>
              <div class="sale-info" v-if="repair.about_me">
                <b>{{ $t('comment') }}</b>
                <p>{{ repair.about_me }}</p>
              </div>
            </div>
        ...
        <div class="product-tools">
          ...
          <div class="product-tools-body">
            ...
            <app-button class="show-phone" type="btn-success" size="btn-lg" @action="handleShowNumber"  v-if="!my">
              <a :href="`tel:${repair.user.phone}`" v-if="isNumber">{{ repair.user.phone }}</a>
              <template v-else>{{ $t('show_number') }}</template>
            </app-button>
            <router-link class="btn-plain order-control-btn" :to="{name: 'TheRepairEdit', params: {id: repair.id}}"   v-if="my">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <button class="btn-plain order-control-btn" v-if="my"   @click="remove(repair.id)" >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default {
  ...
  methods: {
    remove(id){
      API.productAdd.removeRepair({ id: id}).then((res) => {
        if(res.success) {
          this.$router.push('/');
