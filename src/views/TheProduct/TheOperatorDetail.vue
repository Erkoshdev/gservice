        ...
        <div class="product-container">
          <div class="product-container-top">
            ...
            <div class="product-favourite" v-if="$store.getters.loggedIn && !my">
              <input type="checkbox" @change="handleLike(operator)" :checked="operator.is_favourite">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div class="product-container-content">
            <div class="sale-container">
              <div class="product-gallery">
                ...
                >
                  <div class="gallery-item" v-for="(img, imgIdx) in operator.images" :key="imgIdx">
                    <img :src="img.url" alt="">
              ...
              <div class="sale-price">{{ operator.mark.title }} {{ operator.model }}</div>
              <div class="sale-detail-list">
                <div class="sale-detail-item">
                  <div class="sale-detail-name">{{ $t('mark') }}</div>
                  <div class="sale-detail-value">{{ operator.mark.title }}</div>
                </div>
                <div class="sale-detail-item">
                  <div class="sale-detail-name">{{ $t('model') }}</div>
                  <div class="sale-detail-value">{{ operator.model }}</div>
                </div>
                <div class="sale-detail-item">
                  <div class="sale-detail-name">{{ $t('city') }}</div>
                  <div class="sale-detail-value">{{ operator.region.title }}</div>
                </div>
              </div>
              <!--              <div class="sale-info">-->
          ...
          <div class="product-tools-body">
            <div class="owner-badge" v-if="!my">
              ...
              <div class="owner-badge-caption">
                <p>{{ operator.owner.name }}</p>
              </div>
            </div>
            <app-button class="show-phone" type="btn-success" size="btn-lg" v-if="!my" @action="handleShowNumber">
              <a :href="`tel:${operator.owner.phone}`" v-if="isNumber">{{ operator.owner.phone }}</a>
              <template v-else>{{ $t('show_number') }}</template>
            </app-button>
            <router-link class="btn-plain order-control-btn" :to="{name: 'TheDriverEdit', params: {id: operator.id}}"   v-if="my">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <button class="btn-plain order-control-btn" v-if="my"   @click="remove(operator.id)" >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default {
  ...
  methods: {
    remove(id){
      API.productAdd.removeDriver(id).then((res) => {
        if(res.success) {
          this.$router.push('/');
    ...
    fetchData() {
      ...
      API.product.getOperator({params})
          .then((res) => {
            this.operator = res.driver_application;
          })
          .catch((error) => {
            console.log(error);
          })
