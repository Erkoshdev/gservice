        ...
        <div class="product-container">
          <div class="product-container-top">
            ...
            <div class="product-favourite" v-if="$store.getters.loggedIn && !my">
              <input type="checkbox" @change="handleLike(service)" :checked="service.is_favourite">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div class="product-container-content">
            <div class="sale-container">
              <div class="product-gallery">
                ...
                >
                  <div class="gallery-item" v-for="(img, imgIdx) in service.images" :key="imgIdx">
                    <img :src="img.url" alt="">
              ...
              <div class="sale-name">{{ service.type.title }}</div>
              <div class="sale-price">{{ service.mark.title }} {{ service.model }}</div>
              <div class="sale-detail-list">
                <div class="sale-detail-item">
                  <div class="sale-detail-name">{{ $t('mark') }}</div>
                  <div class="sale-detail-value">{{ service.mark.title }}</div>
                </div>
                <div class="sale-detail-item">
                  <div class="sale-detail-name">{{ $t('model') }}</div>
                  <div class="sale-detail-value">{{ service.model }}</div>
                </div>
                <div class="sale-detail-item">
                  <div class="sale-detail-name">{{ $t('city') }}</div>
                  <div class="sale-detail-value">{{ service.driver.region.title }}</div>
                </div>
              </div>
<!--              <div class="sale-info">-->
          ...
          <div class="product-tools-body">
            <div class="owner-badge" v-if="!my">
              ...
              <div class="owner-badge-caption">
                <p>{{ service.driver.name }}</p>
                <span>{{ service.created_at | moment("DD.MM.YYYY HH:mm") }}</span>
              </div>
            </div>
            <app-button class="show-phone" type="btn-success" size="btn-lg" @action="handleShowNumber" v-if="!my">
              <a :href="`tel:${service.driver.phone}`" v-if="isNumber">{{ service.driver.phone }}</a>
              <template v-else>{{ $t('show_number') }}</template>
            </app-button>

            <router-link class="btn-plain order-control-btn" :to="{name: 'TheServiceEdit', params: {id: service.id}}"   v-if="my">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <button class="btn-plain order-control-btn" v-if="my"   @click="remove(service.id)" >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default {
  ...
  methods: {
    remove(id){
      API.productAdd.removeService({ vehicle_id: id}).then((res) => {
        if(res.success) {
          this.$router.push('/');
    ...
    fetchData() {
      ...
      API.product.getService({params})
          .then((res) => {
            this.service = res.vehicle;
          })
          .catch((error) => {
            console.log(error);
          })
