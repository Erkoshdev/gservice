<template>
  <div class="card">
    <div class="card-img">
      ...
      <router-link @click.native="$emit('link')" class="absolute-link" :to="{name: 'TheServiceDetail', params: {id: item.id}}"></router-link>
    </div>
    <div class="card-caption">
      <div class="card-caption-primary">
        <div class="card-caption-primary-left">
          <div class="card-name">
            <router-link @click.native="$emit('link')" :to="{name: 'TheServiceDetail', params: {id: item.id}}">{{ item.type.title }}</router-link>
          </div>
        ...
        <div class="card-caption-primary-right">
          <div class="card-tools" v-if="$store.getters.loggedIn">
            <div class="card-favorite">
              <input type="checkbox" @change="$emit('handleLike', item, 'App\\Models\\Vehicle')" :checked="item.is_favourite">
      ...
      <div class="card-caption-secondary">
        ...
        <app-button type="btn-success" size="btn-sm" @action="handleShowNumber">
          <a :href="`tel:${item.driver.phone}`" v-if="isNumber">{{ item.driver.phone }}</a>
