<template>
  <div class="card">
    <div class="card-img">
      ...
      <router-link @click.native="$emit('link')" class="absolute-link" :to="{name: 'TheShopDetail', params: {id: item.id}}"></router-link>
    </div>
    <div class="card-caption">
      <div class="card-caption-primary">
        <div class="card-caption-primary-left">
          <div class="card-name">
            <router-link @click.native="$emit('link')" :to="{name: 'TheShopDetail', params: {id: item.id}}">{{ item.mark.title }}</router-link>
          </div>
          <div class="card-info-primary">{{ item.category.title }}</div>
          <div class="card-info-secondary">{{ $t('production') }} {{ item.country.title }}</div>
        </div>
        <div class="card-caption-primary-right">
          <div class="card-price" v-if="item.price">{{ item.price | vMask(priceMask) }} ₸</div>
          <div class="card-tools" v-if="$store.getters.loggedIn">
            <div class="card-favorite">
              <input type="checkbox" @change="$emit('handleLike', item, 'App\\Models\\ShopItem')" :checked="item.is_favourite">
